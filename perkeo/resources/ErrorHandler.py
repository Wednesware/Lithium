...
class ErrorHandler:
    ...
    def getParts(self, span: dict) -> tuple[str, str, str]:
        return (
            f"{self.interpreter.perkeo.source.splitlines()[span['line'] - 1][0:span['column'] - 1]}",
            f"{self.interpreter.perkeo.source.splitlines()[span['line'] - 1][span['column'] - 1:span['end_column'] - 1]}",
            f"{self.interpreter.perkeo.source.splitlines()[span['line'] - 1][span['end_column'] - 1:len(self.interpreter.perkeo.source)]}"
        )
