...
class Parser:
    ...
    def parse(self) -> dict[str, any]:
        try:
            ...
            self.eh.throwNoTraceback("internalError", str(err), warning=self.perkeo.getsetting("verbose"))
            raise err
    ...
    def _parse_group(self) -> dict[str, any]:
        ...
        # Allow both grouped expressions and parenthesized call syntax.
        # A single value keeps the original semantics; multiple values are
        # turned into a call where the first item is the target and the rest
        ...
        # Multiple items inside parentheses are parenthesized calls instead of array literals.
        span = self._span_between(open_token, close_token)
        args = self._new_map(span)
        for item in items[1:]:
            self._add_positional(args, item)
        return self._node("group", span, value=self._call(items[0], args, close_token))
    ...
    def _parse_argument_map(self, stop: set[str]) -> dict[str, any]:
        ...
        while not self._at_stop(stop):
            ...
            value = self.parse_expression(stop=stop | {"ARROW"}, allow_implicit_call=False)
            self._add_positional(result, value)
