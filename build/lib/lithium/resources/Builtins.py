...
class Builtins:
    ...
    @staticmethod
    def pctadd(interpreter, target, value: "array") -> dict: # type: ignore
        left, right = Builtins._binaryItems(interpreter, value)
        left_value = Builtins._asNumber(interpreter, left)
        right_value = Builtins._asNumber(interpreter, right)
        result = Builtins._asDecimal(left_value) / Decimal(100) * Builtins._asDecimal(right_value)
        return Builtins._wrapNumber(right_value + result, value["span"])
        
    @staticmethod
    def pctsub(interpreter, target, value: "array") -> dict: # type: ignore
        left, right = Builtins._binaryItems(interpreter, value)
        left_value = Builtins._asNumber(interpreter, left)
        right_value = Builtins._asNumber(interpreter, right)
        result = Builtins._asDecimal(left_value) / Decimal(100) * Builtins._asDecimal(right_value)
        ...
        if sheet_nodes:
            for single_value in sheet_nodes:
                ...
                if not os.path.isfile(pkis_path):
                    interpreter.eh.throw("sheetNotFound", f"could not find import sheet file at \"{pkis_path}\"")
